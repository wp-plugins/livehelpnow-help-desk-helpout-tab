...
if(is_numeric(str_replace("-1","",$meta))) { 
...
?>
<input <?php if(substr_count($id,"clientid") == 1){echo 'style="width: 100px;"';} if(substr_count($id,"livehelpnow_lhnWindowN") == 1){echo 'style="width: 100px;"';} ?> id="<?php echo $id; ?>" name="<?php echo $element_name; ?>" value="<?php echo $meta; ?>" type="<?php echo $type; ?>" class="text large-text <?php echo $class; ?>" />
<?php 
if(substr_count($id,"clientid") == 1){
	echo $numstatus; 
}
...
<script type="text/javascript">
	if(lhnHPPanel===undefined){
	var lhnVersion = 5.3; 
	var lhnAccountN = str_replace("-1","",$clientid);
	if(typeof lhnButtonN == 'undefined'){var lhnButtonN = -1;}
	...
	if(!lhnHPKnowledgeBase && !lhnHPTicketButton && !lhnHPCallbackButton){
	...
	var loadLHNFiles = function () {
		...
		loadLHNFile(lhnJsHost + "www.livehelpnow.net/lhn/js/css/helppanel/" + lhnTheme + "/style.css", "css"); 
	}
...
$new_instance['textarea_id'] = "<script type=\"text/javascript\">
	if(lhnHPPanel===undefined){
	".
	$whiteLabel
	."var lhnVersion = 5.3; 
	var lhnAccountN = ".str_replace("-1","",$new_instance['clientid']).";
	if(typeof lhnButtonN == 'undefined'){var lhnButtonN = -1;}
	var lhnJsHost = ((\"https:\" == document.location.protocol) ? \"https://\" : \"http://\"); 
	var lhnInviteEnabled = ". ($new_instance['livehelpnow_lhnInviteEnabled'] == "on" ? "1" : "0")."; 
	var lhnInviteChime = 0; 
	var lhnWindowN = ". ($new_instance['livehelpnow_lhnWindowN'] == "on" ? "1" : "0") ."; 
	var lhnDepartmentN = 0; 
	var lhnCustomInvitation = ''; 
	var lhnTrackingEnabled = 't';
	...
	var lhnLO_helpPanel_knowledgeBase_find_answers = \"".$new_instance['lhnLO_helpPanel_knowledgeBase_find_answers']."\";
	var lhnLO_helpPanel_knowledgeBase_please_search = \"".$new_instance['lhnLO_helpPanel_knowledgeBase_please_search']."\";
	var lhnLO_helpPanel_typeahead_noResults_message = \"".$new_instance['lhnLO_helpPanel_typeahead_noResults_message']."\";
	var lhnLO_helpPanel_typeahead_result_views = \"".$new_instance['lhnLO_helpPanel_typeahead_result_views']."\";
	if(!lhnHPKnowledgeBase && !lhnHPTicketButton && !lhnHPCallbackButton){
		lhnHPChatButton = true;
		lhnHPPanel = false;
	}
...
$options['livehelpnow_lhnTheme'] = strip_tags( stripslashes($_POST["livehelpnow_lhnTheme"] ) );
$options['livehelpnow_lhnWindowN'] = strip_tags( stripslashes($_POST["livehelpnow_lhnWindowN"] ) );
$options['livehelpnow_lhnInviteEnabled'] = strip_tags( stripslashes($_POST["livehelpnow_lhnInviteEnabled"] ) );

update_option('livehelpnow', $options);
}
$livehelpnow_clientid = $options['livehelpnow_clientid'];

$livehelpnow_lhnTheme = $options['livehelpnow_lhnTheme'];
$livehelpnow_lhnWindowN = $options['livehelpnow_lhnWindowN'];
$livehelpnow_lhnInviteEnabled = $options['livehelpnow_lhnInviteEnabled'];
